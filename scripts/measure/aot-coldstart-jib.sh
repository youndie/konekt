...
AOT_DOCKER="--network ${PROJECT}_default -e KONEKT_DB_URL=jdbc:postgresql://postgres:5432/konekt -e KONEKT_DB_USER=konekt -e KONEKT_DB_PASSWORD=konekt -e KONEKT_JWT_SECRET=dev-secret-not-for-anything-real -e KONEKT_SERVICE=konekt-server -e KONEKT_BROKER_HOST=broker -e KONEKT_BROKER_PORT=9092 -e KONEKT_DEV_REVEAL_OTP=true -e KONEKT_DEV_SCREENS=true -e KONEKT_BRAND=brand-a -e KONEKT_PAYMENT_MOCK_MODE=approve -e KONEKT_PAYMENT_MOCK_DELAY_MS=0"
