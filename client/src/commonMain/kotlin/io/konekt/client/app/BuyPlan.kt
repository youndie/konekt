...
class BuyPlan(
    ...
    suspend fun addressFor(action: KompotAction): String? {
        val plan = (action as? BuyPlanAction)?.planId ?: return null

        // 202 AND NOT 201, and the client must not assume otherwise: the usual answer is a saga
        // waiting for a confirmation. What comes back is an order that exists and is not finished,
        // which is exactly what the order screen is for.
        val order: PurchaseOrderResponse =
            http
                .post(Purchases()) { setBody(CreatePurchaseRequest(plan)) }
                .body()
