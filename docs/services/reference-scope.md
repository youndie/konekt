...
| **Presence in an app store** — no signing, no icon, no launch screen, no store metadata | There is no application to publish yet, and the two open items about that are about *running*, not shipping: [B-85](../backlog/B-85-the-client-has-no-android-target.md) and [B-90](../backlog/B-90-the-ios-build-cannot-leave-the-simulator.md) | Everything an app review needs, none of which exercises a toolkit. This is the row most likely to stay here permanently |
| **More than one server replica** — the realtime bus is `KompotUpdateBroadcaster`, in memory, and the sweeper runs per replica | One instance is the honest configuration for a reference. A shared bus is a dependency and an operational surface; kompot's own reasoning applies, an update is losable because the next screen fetch carries current state | `kompot-realtime-redis` and a claim on the sweeper. The chart guard that makes the boundary enforced rather than implicit is [B-91](../backlog/B-91-a-second-replica-loses-live-updates.md), and it is not built |

## What is *not* on this list, and why

Three things look like they belong here and do not:

- **Android and an iOS device build.** They are work, not boundaries — the multiplatform claim is
  made by this repository and is currently compiled on one platform of the two it names.
  [B-85](../backlog/B-85-the-client-has-no-android-target.md),
  [B-90](../backlog/B-90-the-ios-build-cannot-leave-the-simulator.md).
- **The screens that exist on the server and nowhere else** — the tariff change and the custom
  package builder. A vertical whose only user is an e2e test is unfinished, not scoped out.
