...
| U11 | tracy | the agent publishes no Apple target, so an iOS client cannot log through tracy | [tracy#16](https://github.com/youndie/tracy/issues/16) | fixed in `0.1.13` and verified here — `agent` declares `ios_arm64`, `ios_simulator_arm64` and `ios_x64` in its module metadata and `:client` compiles for both of ours; the issue itself is still open upstream |
