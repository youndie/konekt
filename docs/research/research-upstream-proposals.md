...
| U17 | kompot | `amount_input` always spaces the symbol away from the number, and `$50` has no space | [kompot#99](https://github.com/youndie/kompot/issues/99) | closed, released in `0.34.0.97` as `currencySpaced`; konekt sets it from `MoneyFormat` since 2026-09-02 |
