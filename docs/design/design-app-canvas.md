...
| The platform's typeface, at Material's sizes | not since `B-114`. Manrope carries the text and Space Grotesk the figures, bundled as static instances per weight under `client/src/commonMain/composeResources/font/` (OFL, licences in `docs/design/fonts/`), and `KonektTypography` is the one scale — kompot resolves every token from `MaterialTheme.typography`, so it is the only place a size or weight lives. Static and unhinted because a variable file is instanced by the platform before Skia sees it, and two platforms do not agree to the pixel. |
| A white page with tinted cards | the canvas is the other way round — a mint-grey page (`background`) with near-white cards (`surface`) and `surface_variant` for chips and the back control. The kit always carried all three; the client drew the page in the card's colour and the cards in the chip's (`B-114` G2). |
| A plan page as a title, a price and a list of rows | the canvas's plan page: a tinted hero (`surface` in the `accent` tone) with the label, the price and the quota as the figure, the validity and the zone as chips (`surface` at `chip` density); a white table of what is included (`surface` with `dividers`); the activation note; and `Charged once` with the buy button in a `pinned` surface that stays above the bar (`B-114`, block 2). |
| A purchase outcome as a banner and an order row | the canvas's outcome: a mark in a disc (`icon` — a check in `primary`, a cross in `error`), a `headline_small` — `Paid.` / `Payment failed.` — one paragraph, and a receipt as a table (`surface` with `dividers`): Order, Charged, Balance left; or Reference and Balance on a refusal (`B-114`, block 2). |
| An order's status as a word under the amount | a chip on the right of the title (`B-114`, block 4) — mint for a purchase that went through, red-tinted for money that came back, grey for the rest — drawn through the registry as a `surface` chip in the status's tone; the signed amount stays, on the second line. The selected filter is the tonal pill and the rest are outlined. The canvas's active-package card with its bar and remainder is a model question (an order as the package it bought) and is not drawn. |
| The profile as captions and a primary pill for leaving | the number as the header with its label above (no name and no initials exist, `B-55`), what the line holds as a white settings card with dividers, support as the mint card with a heading — without `Open chat`, since there is no chat — and `Sign out` as a red text row in its own card at the foot (`B-114`, block 4). |
| Filled tinted fields; two outlined buttons above `Sign in` | fields are outlined on the page (`B-114`, block 4); on the code step `Sign in` is the first and only pill, and the way to a new code and back to the number are text links under it. The resend confirmation stays, because it is drawn only after a resend, and a resend that changes nothing on screen is indistinguishable from a button that does not work. |
| The confirmation as a sheet over the plan page, opening on a hold banner | the sheet's content on a screen of its own (`B-114`, block 5): `Confirm purchase`, the plan and the price as a table, `Pay from` over the one source drawn as the chosen option, `Pay $X`, the hold sentence under the button, `Not now` as a link. The consent checkbox is not drawn — there are no terms to accept — and the card option is not drawn because there is no card (`B-40`). The sheet itself is a presentation the client does not have yet. |
| Cards at one radius and one inset, everywhere | they are not, since `B-112`. The canvas pairs a radius with an inset as a SCALE — `36/22` for a screen's headline blocks (Balance, Smart 20, Data remaining), `md` with `18` for the items in a list, `md` with `16` for a notice — and this build drew `20/16` for all six of its card renderers. `CardGeometry` holds the three tiers; the RADIUS half comes from the brand's own scale (`sm 12/8 | md 20/12 | lg 36/22`, this document's token block) so a rebrand moves it, and the INSET half is layout and is ours. Where a brand-A mockup draws a list card at 22 — brand B's large — the declared token wins: a token block is what a design says, a mockup is a picture of one screen. |
...
| a `Roaming` button beside `Buy a package` | drawn since `B-114` (block 3) — as the canvas's pair under the counters, a tonal pill and an outlined one — but it opens the travel screen (`app://roaming`, the one `B-88` gave the packages), not a filter of the catalogue: the catalogue is one list and roaming plans are in it, and filtering it is [B-57](../backlog/B-57-plans-catalogue-against-section-02.md)'s. It replaced the `Travel packages` banner, which was the same door in a different shape. |
